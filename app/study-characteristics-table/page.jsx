...
import { Table, Col, Row, Select, Button, Tooltip, Input, Layout, Spin } from "antd";
...
import { LeftOutlined, RightOutlined, FilterOutlined, TableOutlined } from "@ant-design/icons";

const { Option } = Select;
const { Sider, Content } = Layout;

const App = () => {
  const [checkedList, setCheckedList] = useState(columns.slice(0, 5).map(({ key }) => key));
  ...
  const newColumns = columns.map((item) => {
    const ellipsisConfig = typeof item.key === "string" && item.key.length > 10
      ? {
          ellipsis: {
            showTitle: false,
          },
      ...
      Object.entries(defferedFilters).every(([filterKey, filterValues]) => {
        if (filterValues.includes("All")) {
          ...
          return filterValues.some((filterValue) =>
            item[filterKey].toLowerCase().includes(filterValue.trim().toLowerCase())
          );
        }
      })
    );
  ...
  const distinctNCTCount = new Set(filteredData.map((item) => item["NCT"])).size;
  ...
  return (
    <div className="container">
      ...
      <Row gutter={10}>
        <Layout style={{ minHeight: "50vh", display: "flex", padding: "0 10px 0 10px" }}>
          <Sider
            ...
            width={200}
            collapsedWidth={30}
            style={{ overflow: "auto", maxHeight: leftCollapsed ? "auto" : "50vh", left: 0 }}
          >
            {!leftCollapsed ? (
              <div
                style={{
              ...
              >
                <Button type="link" onClick={toggleLeftCollapsed} style={{ padding: 0 }}>
                  <LeftOutlined />
          ...
          >
            {!rightCollapsed ? (
              ...
              >
                Column / Filter
                <Button type="link" onClick={toggleRightCollapsed} style={{ padding: 0 }}>
                  <TableOutlined style={{ fontSize: "15px" }} />
                  <RightOutlined />
                </Button>
                <MultiColumnSelector checkedList={checkedList} setCheckedList={setCheckedList} />
              </div>
            ) : (
              <div
                style={{
              ...
              >
                <Button type="link" onClick={toggleRightCollapsed} style={{ padding: 0, textAlign: "center" }}>
                  <LeftOutlined />
                  <br />
                  <TableOutlined style={{ fontSize: "15px" }} />
                </Button>
